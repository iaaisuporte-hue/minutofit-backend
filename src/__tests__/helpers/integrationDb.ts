...
/** Prefixo dos dados criados pelos testes — tudo é removido no fim. */
export const FIXTURE_TAG = 'itest-perf';
...
/** Remove tudo que os testes criaram. Seguro de rodar antes e depois. */
export async function cleanFixtures(c: Client): Promise<void> {
  await c.query(`DELETE FROM users WHERE email LIKE $1`, [`${FIXTURE_TAG}-%@test.local`]);
  await c.query(`DELETE FROM exercises WHERE source = $1`, [FIXTURE_TAG]);
}

let seq = 0;
/** Usuário de teste com os NOT NULL do schema legado preenchidos. */
export async function createUser(c: Client, label: string): Promise<number> {
  seq += 1;
  ...
  const { rows } = await c.query(
    ...
    [`${FIXTURE_TAG}-${label}-${unique}@test.local`, `Fixture ${label}`, unique, `119${unique}`],
  );
  return rows[0].id;
}

export async function createExercise(c: Client, name: string): Promise<string> {
  const { rows } = await c.query(
    `INSERT INTO exercises (source, name, normalized_name)
     VALUES ($1, $2, $3) RETURNING id`,
    [FIXTURE_TAG, name, name.toLowerCase()],
  );
  return rows[0].id;
}
